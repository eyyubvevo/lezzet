            ...
            <li>
                ...
                <ul class="dropdown-container">
                    @foreach($categories as $category)
                        <li>
                            <a href="{{route(app()->getLocale().'.home.products',['slug' => $category->slug])}}">{{$category->name}}</a>
                        </li>
                    @endforeach
                </ul>
            </li>
    ...
    <div class="container-fluid">
        <div class="row align-items-center">
            <div class="col-xl-2 col-lg-2 col-md-2 col-sm-2 col-2">
                <a class="logo-header" href="{{route(app()->getLocale().'.home')}}"><img src="{{asset('storage/'.setting('logo'))}}"
                                                                     alt="{{setting('title')}}"></a>
            </div>
                        ...
                        <div class="dropdown-language__content">
                            ...
                            @php($route=implode(".", array_slice(explode(".", \Request::route()->getName()), 1)))
                                ...
                                <a rel="alternate" hreflang="{{ $available_locale }}"
                                       href="/language/{{ $available_locale }}/home">
                                        {{ $available_locale }}
                                    </a>
                                @else
                                @if(@$slug1 && @!@$slug2)
                                 @php($a=$slug1->getTranslation('slug', $available_locale))
                                    <!-- <a rel="alternate" hreflang="{{ $available_locale }}"
                                       href="/language/{{ $available_locale }}/{{$route}}/{{$a}}">
                                        {{ $available_locale }}
                                    </a> -->
                                    <a rel="alternate" hreflang="{{ $available_locale }}"
                                         href="{{route($available_locale.'.'.$route,$a)}}">
                                        {{ $available_locale }}
                                    </a>
                                    <!-- <a rel="alternate" hreflang="{{ $available_locale }}"
                                         href="{{route(app()->getLocale().'.'.$route,$a)}}">
                                        {{ $available_locale }}
                                    </a> -->
                                ...
                                @elseif (@$slug2)
                                    ...
                                    <?php 
                                    $a=$slug1->getTranslation('slug', $available_locale);
                                    $b=$slug2->getTranslation('slug', $available_locale);
                                    ?>

                                    <a rel="alternate" hreflang="{{ $available_locale }}"
                                         href="{{route($available_locale.'.'.$route,[$a,$b])}}">
                                        {{ $available_locale }}
                                    </a>

                                    <!-- <a rel="alternate" hreflang="{{ $available_locale }}"
                                       href="/language/{{ $available_locale }}/{{$route}}/{{$a}}/{{$b}}">
                                        {{ $available_locale }}
                                    </a> -->
                                @else
                                ...
                                <!-- <a rel="alternate" hreflang="{{ $available_locale }}"
                                    ...
                                    <a rel="alternate" hreflang="{{ $available_locale }}"
                                         href="{{route($available_locale.'.'.$route)}}">
                                        {{ $available_locale }}
                                    </a>
                                @endif
                                @endif
